;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         wpboilerplate Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('wpboilerplate_about_us_widget') != 1){
                prefetch();
            }
        });

        $('body').off('click','.wpboilerplate_flogo_uploader');

        $('body').on('click','.wpboilerplate_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.wpboilerplate-logo-preview');
                ...
                $(el).siblings().children('.wpboilerplate_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.wpboilerplate_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.wpboilerplate_logo_id').val());
                var atts = $(el).siblings().children('.wpboilerplate_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.wpboilerplate_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.wpboilerplate-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.wpboilerplate_flogo_uploader').val('Change Image');
