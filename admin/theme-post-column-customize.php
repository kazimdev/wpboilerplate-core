...
if (!class_exists('wpboilerplate_Post_Column_Customize')) {
	class wpboilerplate_Post_Column_Customize
	{
		//$instance variable
		...
		public function edit_service_columns($columns)
		{
			...
			$columns['title'] = esc_html__('Title', 'wpboilerplate-core');
			$columns['thumbnail'] = '<a href="edit.php?post_type=service&orderby=title&order=' . urlencode($order) . '">' . esc_html__('Thumbnail', 'wpboilerplate-core') . '</a>';
			...
			$columns['date'] = esc_html__('Date', 'wpboilerplate-core');
			return $columns;
		}
		...
		{
			$columns['icon'] = esc_html__('Icon', 'wpboilerplate-core');
			return $columns;
		}
		...
		{
			$post_term_meta = get_term_meta($post_id, 'wpboilerplate_service_category', true);
	...
	if (class_exists('wpboilerplate_Post_Column_Customize')) {
		wpboilerplate_Post_Column_Customize::getInstance();
	}
}
